...
%--------------------------------------------------------------------------
    ...
    % Model
    % model = [
    %     4 37 0.5; %"G_shadow_map_ITU_R_M2135_UMa" "G_shadow_map_3GPPTR38_901_UMa"
    %     6 50 0.5; %"G_shadow_map_ITU_R_M2135_UMa" "G_shadow_map_3GPPTR38_901_UMa"
    %     3 10 0.5; %"G_shadow_map_ITU_R_M2135_UMi"
    %     4 13 0.5; %"G_shadow_map_ITU_R_M2135_UMi"
    %     7 7 0.5; %"G_shadow_map_ITU_R_M2135_UMi"
    %     4 10 0.5; %"G_shadow_map_3GPPTR38_901_UMi"
    %     7.82 13 0.5; %"G_shadow_map_3GPPTR38_901_UMi"
    %     8 50 0.5; %"G_shadow_map_3GPPTR36_814_Case_1"
    %     3.34 37 0.5; %"G_shadow_map_3GPPTR36_777_UMa_AV_50m" % 4.64*exp(-0.0066*50)
    %     2.40 37 0.5; %"G_shadow_map_3GPPTR36_777_UMa_AV_100m" % 4.64*exp(-0.0066*100)
    %     1.23 37 0.5; %"G_shadow_map_3GPPTR36_777_UMa_AV_200m" % 4.64*exp(-0.0066*200)
    %     0.64 37 0.5; %"G_shadow_map_3GPPTR36_777_UMa_AV_300m" % 4.64*exp(-0.0066*300)
    %     2 10 0.5; %"G_shadow_map_3GPPTR36_777_UMi_AV_300m" LoS %
    %     8 13 0.5]; %"G_shadow_map_3GPPTR36_777_UMi_AV_300m" NLoS %
    ...
    for model_index = 1:size(model,1)
        ...
        sigma_shadow_dB = model(model_index,1);
        sigma_shadow_vector_dB = ones(number_of_maps_per_sigma_plus,1) * sigma_shadow_dB;
        autocorrelation_m = model(model_index,2);
        autocorrelation_vector_m = ones(number_of_maps_per_sigma_plus,1) * autocorrelation_m;
        crosscorelation = model(model_index,3);
        str_sigma = replace(num2str(sigma_shadow_dB),'.','_');
        str_auto = replace(num2str(autocorrelation_m),'.','_');
        
        % Create seeds
        scenario_seed = 1; %scenario seed
        seed_vector = (scenario_seed * 1000 + (1:1:number_of_maps_per_sigma_plus) + 1)';
        ...
        G_shadow_map_dB = zeros(size(G_shadow_temp,1), size(G_shadow_temp,2), number_of_maps_per_sigma);
            
        % Select the common map for cross-correlation
        G_shadow_map_macro_common_dB = G_shadow_temp(:, :, 1); % We set the first map as common map for cross-correlation
        ...
        for map_within_sigma = 1:number_of_maps_per_sigma
            ...
            G_shadow_map_dB(:,:, map_within_sigma) = crosscorelation*sqrt(2)*G_shadow_map_macro_common_dB + (1-crosscorelation)*sqrt(2)*G_shadow_temp(:, :, map_within_sigma+1);
        
        end 
        
        % Plot figures
        figure(100+model_index)
        clf
        %colormap('jet')
        ...
        if(saveFig == 1)
            str_saveas = sprintf('f_shadowing_sigma_%s_std_%s', str_sigma, str_auto);
        ...
        PDF = pdf(G_shadow_map_dB(:,:,2), PDF_values_dB);
        std_dev_dB = mean(std(G_shadow_map_dB(:,:,2)));
        %std_dev_dB = 10*log10(mean(std(G_shadow_map_dB(:,:,index_first_of_batch+1))));
        figure(200+model_index);
        subplot(2,1,1);
        ...
        G_shadow_map_dB = single(round(G_shadow_map_dB + randn(scenario_elements(1),scenario_elements(2), number_of_maps_per_sigma)*0.0001,2));

        %% Save results
        str_saveas = sprintf('G_shadowing_sigma_%s_std_%s.mat', str_sigma, str_auto);
