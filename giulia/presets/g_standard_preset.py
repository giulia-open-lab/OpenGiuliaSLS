...
class Giulia:
    ...
    def help(self):
        ...
        print("    '3GPPTR36_814_Case_1_omni_dana',")
    ...
    def __init__(self,
        ...
        # Define placeholders for attributes
        ...
        self.simulation_config_obj.set_sn_indicator(
            self.network_deployment_obj)
        self.simulation_config_obj.set_instantaneous_RSRP(
            self.network_deployment_obj)
        ...
        self.mcs_resource_obj = \
            mcs_resources.MCS_Resource(self.simulation_config_obj)
    ...
    def configure(self, snapshot_index, max_time_ms):
        ...
        self.distance_angles_ue_to_cell_obj = distances_angles.Distance_Angles_ue_to_cell(self.simulation_config_obj,
                                                        self.site_deployment_obj,
                                                        self.network_deployment_obj,
                                                        self.ue_deployment_obj)
        ...
        self.dl_fast_fading_gain_ueAnt_to_cellAnt_obj = \
            fast_fading_gains.Fast_Fading_Gain(self.simulation_config_obj,
                                               ...
                                               self.array_steering_vector_ue_to_cell_obj,
                                               self.K_factor_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.dl_fast_fading_gain_ueAnt_to_cellAnt_obj, 'process', scheduling_info["dl_fast_fading_gain_ueAnt_to_cellAnt_obj"])
        ...
        self.channel_gain_ue_to_cell_obj = \
            channels.Channel(self.simulation_config_obj,
                             ...
                             self.dl_fast_fading_gain_ueAnt_to_cellAnt_obj,
                             self.channel_sn_obj)
        self.eventDriven_obj.add_event(0, self.channel_gain_ue_to_cell_obj, 'process', scheduling_info["channel_gain_ue_to_cell_obj"])
        ...
        self.SSB_precoded_channel_gain_no_fast_fading_ue_to_cell_obj = \
            precoded_channel_gains.Precoded_Channel_Gain_SSB_no_fast_fading_ue_to_cell(
                ...
                self.los_channel_gain_ue_to_cell_obj,
                self.array_steering_vector_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.SSB_precoded_channel_gain_no_fast_fading_ue_to_cell_obj, 'process', scheduling_info["SSB_precoded_channel_gain_no_fast_fading_ue_to_cell_obj"])
        ...
        self.SSB_precoded_channel_gain_ue_to_cell_obj = \
            precoded_channel_gains.Precoded_Channel_Gain_SSB_ue_to_cell(
                ...
                self.ue_deployment_obj,
                self.channel_gain_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.SSB_precoded_channel_gain_ue_to_cell_obj, 'process', scheduling_info["SSB_precoded_channel_gain_ue_to_cell_obj"])
        ...
        self.CSI_RS_precoded_channel_gain_ue_to_cell_obj = \
            precoded_channel_gains.Precoded_Channel_Gain_CSI_RS_ue_to_cell(
                ...
                self.ue_deployment_obj,
                self.channel_gain_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.CSI_RS_precoded_channel_gain_ue_to_cell_obj, 'process', scheduling_info["CSI_RS_precoded_channel_gain_ue_to_cell_obj"])
        ...
        self.SSB_RSS_per_PRB_ue_to_cell_obj = \
            calculate_rss.RSS_SSB(
                ...
                self.cell_SSB_conf_obj,
                self.SSB_precoded_channel_gain_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.SSB_RSS_per_PRB_ue_to_cell_obj, 'process', scheduling_info["SSB_RSS_per_PRB_ue_to_cell_obj"])
        ...
        self.CSI_RS_RSS_per_PRB_ue_to_cell_obj = \
            calculate_rss.RSS_CSI_RS(
                ...
                self.cell_CSI_RS_conf_obj,
                self.CSI_RS_precoded_channel_gain_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.CSI_RS_RSS_per_PRB_ue_to_cell_obj, 'process', scheduling_info["CSI_RS_RSS_per_PRB_ue_to_cell_obj"])
        ...
        ##### CRS - LTE TYPE ####
        # Calculates RSRP considering only the antenna pattern (of element or antenna) - This does NOT include array gains, e.g. SBB or CSI-RS precoding/beams
        # This is a benchmark
        self.CRS_RSRP_no_fast_fading_ue_to_cell_obj = \
            calculate_rsrp.RSRP_CRS_no_fast_fading(
                'CRS_RSRP_no_fast_fading',
                self.network_deployment_obj,
                None,
                self.slow_channel_gain_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.CRS_RSRP_no_fast_fading_ue_to_cell_obj, 'process', scheduling_info["CRS_RSRP_no_fast_fading_ue_to_cell_obj"])
        ...
        self.SSB_RSRP_ue_to_cell_obj = \
            calculate_rsrp.RSRP_SSB(
                ...
                self.cell_SSB_conf_obj,
                self.SSB_RSS_per_PRB_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.SSB_RSRP_ue_to_cell_obj, 'process', scheduling_info["SSB_RSRP_ue_to_cell_obj"])
        ...
        self.CSI_RS_RSRP_ue_to_cell_obj = \
            calculate_rsrp.RSRP_CSI_RS(
                ...
                self.cell_CSI_RS_conf_obj,
                self.CSI_RS_RSS_per_PRB_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.CSI_RS_RSRP_ue_to_cell_obj, 'process', scheduling_info["CSI_RS_RSRP_ue_to_cell_obj"])
        ...
        ##### CRS - LTE TYPE ####
        # In this case, the UE connects to the cell that provides the best RSRP without considering SSB array gains
        # In other words, the antenna gain is only that of the element/array derived from the antenna patterns
        # This is a benchmark
        self.best_serving_cell_ID_per_ue_based_on_CRS_obj = \
            cell_selections.Cell_Selection_CRS(
                'strongest_rsrp',
                self.simulation_config_obj,
                self.ue_playground_deployment_obj,
                self.ue_deployment_obj,
                ...
                self.CRS_RSRP_no_fast_fading_ue_to_cell_obj,
                self.dl_noise_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.best_serving_cell_ID_per_ue_based_on_CRS_obj, 'process', scheduling_info["best_serving_cell_ID_per_ue_based_on_CRS_obj"]["process"])
        ...
        self.best_serving_cell_ID_per_ue_based_on_SSB_obj = \
            cell_selections.Cell_Selection_SSB(
                ...
                self.SSB_RSRP_no_fast_fading_ue_to_cell_obj,
                self.dl_noise_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.best_serving_cell_ID_per_ue_based_on_SSB_obj, 'process', scheduling_info["best_serving_cell_ID_per_ue_based_on_SSB_obj"]["process"])
        ...
        ##### CRS - LTE TYPE ####
        self.cell_reselection_based_on_CRS_obj = \
            cell_re_selections.Cell_Re_Selection_CRS(
                ...
                self.best_serving_cell_ID_per_ue_based_on_CRS_obj,
                self.CRS_RSRP_no_fast_fading_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.cell_reselection_based_on_CRS_obj, 'process', scheduling_info["cell_reselection_based_on_CRS_obj"])
        ...
        self.cell_reselection_based_on_SSB_obj = \
            cell_re_selections.Cell_Re_Selection_SSB(
                ...
                self.best_serving_cell_ID_per_ue_based_on_SSB_obj,
                self.SSB_RSRP_no_fast_fading_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.cell_reselection_based_on_SSB_obj, 'process', scheduling_info["cell_reselection_based_on_SSB_obj"])
        ...
        ##### CRS - LTE TYPE ####
        self.eventDriven_obj.add_event(0, self.best_serving_cell_ID_per_ue_based_on_CRS_obj, 'calculate_server_stats', scheduling_info["best_serving_cell_ID_per_ue_based_on_CRS_obj"]["calculate_server_stats"])

        ##### SBB ####
        self.eventDriven_obj.add_event(0, self.best_serving_cell_ID_per_ue_based_on_SSB_obj, 'calculate_server_stats', scheduling_info["best_serving_cell_ID_per_ue_based_on_SSB_obj"]["calculate_server_stats"])
        ...
        self.best_serving_CSI_RS_per_ue_obj = \
            calculate_best_serving_beam.Beam_Selection(
                ...
                self.best_serving_cell_ID_per_ue_based_on_SSB_obj)
        self.eventDriven_obj.add_event(0, self.best_serving_CSI_RS_per_ue_obj, 'process', scheduling_info["best_serving_CSI_RS_per_ue_obj"])
        ...
        ##### CRS - LTE TYPE ####
        self.CRS_sinr_ue_to_cell_obj = \
            calculate_sinr.SINR_CRS(
                ...
                self.best_serving_cell_ID_per_ue_based_on_CRS_obj,
                None)
        self.eventDriven_obj.add_event(0, self.CRS_sinr_ue_to_cell_obj, 'process', scheduling_info["CRS_sinr_ue_to_cell_obj"])
        ...
        self.SSB_sinr_ue_to_cell_obj = \
            calculate_sinr.SINR_SSB(
                ...
                self.best_serving_cell_ID_per_ue_based_on_SSB_obj,
                None)
        self.eventDriven_obj.add_event(0, self.SSB_sinr_ue_to_cell_obj, 'process', scheduling_info["SSB_sinr_ue_to_cell_obj"])
        ...
        self.base_stations_obj = \
            base_stations.BaseStations(                
                ...
                self.dl_fast_fading_gain_ueAnt_to_cellAnt_obj, 
                self.SSB_sinr_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.base_stations_obj, 'process', scheduling_info["base_stations_obj"])        
        ...
        self.CSI_RS_sinr_per_PRB_ue_to_cell_obj = \
            calculate_sinr.SINR_CSI_RS(
                ...
                self.mi_obj,
                self.bler_vs_sinrs_lut_obj)
        self.eventDriven_obj.add_event(0, self.CSI_RS_sinr_per_PRB_ue_to_cell_obj, 'process', scheduling_info["CSI_RS_sinr_per_PRB_ue_to_cell_obj"])
        ...
        self.ue_throughput_based_on_ins_CSI_RS_SINR_per_PRB_obj = \
            calculate_rate.Rate_based_on_ins_CSI_RS_SINR(
                ...
                None,
                self.CSI_RS_sinr_per_PRB_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.ue_throughput_based_on_ins_CSI_RS_SINR_per_PRB_obj, 'process', scheduling_info["ue_throughput_based_on_ins_CSI_RS_SINR_per_PRB_obj"])
        ...
        self.ue_throughput_based_on_eff_CSI_RS_SINR_per_PRB_obj = \
            calculate_rate.Rate_based_on_eff_CSI_RS_SINR(
                ...
                self.base_stations_obj,
                self.CSI_RS_sinr_per_PRB_ue_to_cell_obj)
        self.eventDriven_obj.add_event(0, self.ue_throughput_based_on_eff_CSI_RS_SINR_per_PRB_obj, 'process', scheduling_info["ue_throughput_based_on_eff_CSI_RS_SINR_per_PRB_obj"])
        ...
        self.performance_obj = \
            save_performance.Performance(
                ...
                self.ue_throughput_based_on_eff_CSI_RS_SINR_per_PRB_obj,
                self.power_consumption_obj)
        self.eventDriven_obj.add_event(0, self.performance_obj, 'process', scheduling_info["performance_obj"])


        #### Mobility  ####
        #####################################

        # Calculate UE position based on UE mobility model
        if self.simulation_config_obj.ue_mobility != None:
            ue_mobility_obj = \
                mobilities.Mobility(
                    ...
                    self.ue_playground_deployment_obj,
                    self.ue_deployment_obj)
            self.eventDriven_obj.add_event(0, ue_mobility_obj, 'process', scheduling_info["ue_mobility_obj"])
            
        # Calculate BS position based on BS mobility model
        if self.simulation_config_obj.bs_mobility != None:
            bs_mobility_obj = \
                mobilities.Mobility(
                    self.simulation_config_obj,
                    self.site_deployment_obj,
                    self.network_deployment_obj)
            self.eventDriven_obj.add_event(0, bs_mobility_obj, 'process', scheduling_info["bs_mobility_obj"])            
